...
//    DataB = IMABOW - (Size_H >> 1);
   ...
   //     );
    ...
    randomMatrix_particle = (float *)malloc(3 * NUM_PARTICLE * sizeof( float ));
    ...
    step_index = (NUM_PSEUDORANDOM - min_index) / (3 * NUM_PARTICLE);
    max_index = 3 * NUM_PARTICLE * step_index + min_index;
    ...
    else
    {
        for(inum = 0; inum < NUM_PARTICLE; inum++ )
        {
            SW[inum] = Size_W + randomMatrix_particle[2*NUM_PARTICLE + inum];
    ...
    {
        particle_templates_line = particle_templates + jnum * NUM_TEMPLATE_PIXEL;

        SampleCH = Center_H + (signed short int)(randomMatrix_particle[jnum] * affsig[1]);
        SampleCW = Center_W + (signed short int)(randomMatrix_particle[NUM_PARTICLE + jnum] * affsig[0]);



//        SampleSW = Size_W; // + randomMatrix_particle[2*NUM_PARTICLE + jnum];
//        SampleSH = Size_H;//SampleSW * ratio_h_w;
        SampleSW = SW[jnum];
        SampleSH = SH[jnum];
      ...
      //  SampleCW = (SampleCW < DataL) ? DataL : (SampleCW > DataR) ? DataR : SampleCW;
        ...
        particle_data[jnum] = SampleCH;
        particle_data[NUM_PARTICLE + jnum] = SampleCW;
        particle_data[2 * NUM_PARTICLE + jnum] = (SampleSH);
        particle_data[3 * NUM_PARTICLE + jnum] = (SampleSW);
...
//shixiaopanduan
//***************************************************************************************
void IEinit(IE &ie,int frame,TargetData_TYPE target)   //
{
    ie.error_flag=0;
    ie.prepush_acc=0;
    ie.prepush_flag=0;
    ...
    ie.tt[(frame - 1) % 7] = target;
}

void IEupd(IE &ie,int frame,TargetData_TYPE target)
{
    ie.tt[(frame - 1) % 7] = target;

   // TargetCenterDistanceXSave = ie.xmove[(frame - 2) % 20];
   // TargetCenterDistanceYSave = ie.ymove[(frame - 2) % 20];
    ie.xmove[(frame-2)%20] = target.TCenter_W - ie.tt[(frame - 2) % 7].TCenter_W;
    ie.ymove[(frame-2)%20] = target.TCenter_H - ie.tt[(frame - 2) % 7].TCenter_H;

}


void IEpre(IE &ie,float ncc_max,int frame,TargetData_TYPE target)
{
    short int SumOfTargetCenterDistanceX = 0,i;
    short int SumOfTargetCenterDistanceY = 0;
    TargetData_TYPE LastTarget;  // 记录前一帧目标的坐标信息
    float Targetdx,Targetdy;
    unsigned short int ForcastTarCenterX,ForcastTarCenterY;
    //   跟踪失效判断和预推
     if(ie.prepush_flag == 1 && ncc_max>0.5)  // 丢失后重新检测
     {
         ie.prepush_acc++;  // 记录目标重新出现时，ncc算法抓到目标的连续帧数
     }
     else
     {
         ie.prepush_acc = 0;
     ...
     if(frame < 10)
     {
         if(ncc_max<0.5)  // 如果前10帧中目标的ncc值小于0.5，则认为跟踪失败，启动检测
         {
             ie.error_flag = 1;
         }
     }
     else
     {
         // 预推结果
         // 1.计算连续5帧目标的偏移量
         SumOfTargetCenterDistanceX = 0;
         SumOfTargetCenterDistanceY = 0;
         for ( i = frame-3;i > frame - 8; i--)
         {
             SumOfTargetCenterDistanceX += ie.xmove[i%20];//计算正常跟踪时连续5帧的中心点位移量
             SumOfTargetCenterDistanceY += ie.ymove[i%20];
         }
         Targetdx = 1.0 * SumOfTargetCenterDistanceX/5;
         Targetdy = 1.0 * SumOfTargetCenterDistanceY/5;

         // 2.预推本帧位置：直接利用前5帧目标的偏移量（速度）进行预测
         LastTarget = ie.tt[ ( frame - 2 ) % 7]; // 前一帧目标的坐标信息
         ForcastTarCenterX = LastTarget.TCenter_W + Targetdx + 0.5;  // 预测得到本帧的目标位置
         ForcastTarCenterY = LastTarget.TCenter_H + Targetdy + 0.5;

         // 3.计算测量值和预测值两者之间的误差较小，且测量值的ncc结果较大，则以测量值为准
         if((abs(target.TCenter_W - ForcastTarCenterX)<= 3
                 && abs(target.TCenter_H - ForcastTarCenterY)<=3 && ncc_max > 0.3)  // 这里添加了NCC值的设定
                 || (ncc_max > 0.8))
         {
             //target.TCenter_W = target.TCenter_W;
             //target.TCenter_H = target.TCenter_H;
             ie.prepush_rnum = 0;
             ie.prepush_flag = 0;
         }
         else if((abs(target.TCenter_W - ForcastTarCenterX)<= 3 || abs(target.TCenter_H - ForcastTarCenterY)<= 3) && (ncc_max > 0.5))
         {
             target.TCenter_W = (target.TCenter_W + ForcastTarCenterX)/2 + 0.5;
             target.TCenter_H = (target.TCenter_H + ForcastTarCenterY)/2 + 0.5;
             ie.prepush_rnum = 0;
             ie.prepush_flag = 0;
         }
         else
         {
             target.TCenter_W = ForcastTarCenterX;
             target.TCenter_H = ForcastTarCenterY;
             ie.prepush_rnum++;
             ie.prepush_flag = 1;
         }
         // 修改本帧偏移量和其他信息
         ie.xmove[(frame -2)%20] = target.TCenter_W - LastTarget.TCenter_W;
         ie.ymove[(frame -2)%20] = target.TCenter_H - LastTarget.TCenter_H;
         ie.tt[(frame -1)%7] = target;
         if(ie.prepush_rnum > 15)  // 预推时间由30变成15
         {
             ie.error_flag = 1;
         }
         if(ie.prepush_acc > 5)   // 目标重新出现并找到
         {
             ie.prepush_flag = 0;
             ie.prepush_rnum = 0;
         }
